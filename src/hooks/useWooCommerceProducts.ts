  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (invokeError) {
        ...
        // Tentamos extrair a mensagem de erro da Edge Function se ela estiver no payload de dados
        const edgeFunctionError = data?.error || invokeError.message;
        throw new Error(edgeFunctionError); 
      }
      
      if (data.error) {
        // Se a Edge Function retornou um erro (ex: chaves não configuradas, erro 401/404)
      ...
      let userMessage = 'Erro de conexão com o servidor. Verifique a URL e as chaves no Supabase Secrets.';
      
      // Tratamento de erros específicos (agora mais detalhado)
      ...
      } else if (errorMessage.includes('Network/DNS Error')) {
          // Se for erro de rede, exibe a mensagem completa que inclui a URL base
